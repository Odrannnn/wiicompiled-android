...
- Android pipeline-cache reconstruction adapts to the live frame budget: slow frames pause background prewarming, stable frames restore it gradually, and thermal pressure caps it independently. Urgent first-use shaders keep normal-priority workers. The graphics settings page exposes frame-time percentiles, presentation-stage timings, shader queue state, thermal status, and Android performance-hint availability.
...
- A touch GameCube pad supplies analog steering and the main buttons; Mario Kart passes controller detection and reaches the main menu without a physical controller. Bluetooth gamepads are routed by device to four persistent controller ports for local split-screen play. The port-1 steering fallback is isolated from ports 2–4, and controller add/remove is synchronized with the game thread so hotplug cannot close a pad during a frame. Sensor-capable SDL gamepads can turn a quick controller rotation into the D-pad Up pulse used for wheelies and tricks; the controller menu exposes an enable switch and threshold. An Android **Settings** overlay button opens the in-game settings bar for resolution, graphics, audio, controller assignment, button mapping, and motion controls without an F10 keyboard. On Android, the overlay uses the framebuffer's short side as a fallback scale for high-resolution tablets, respects the live system-bar/cutout safe area, and refreshes those insets after keyboard dock and display changes.
- Retro-WFC keeps the signed Retro Rewind payload's login request compatible with the live service by routing the legacy `nas.play.rwfc.net` target and HTTP Host header through `play.rwfc.net`. This targets the observed error-20100 login failure without changing unrelated Wii networking.
- Disc extraction, private-runtime builds, and Retro Rewind installation run as cancellable foreground services. They survive page changes, activity recreation, display docking, and normal backgrounding, and they refuse to overlap operations that mutate the same private data. The builder continuously reserves working storage, pauses during severe thermal pressure, and explains signal exits such as 133/SIGTRAP and 134/SIGABRT with the last compiler output.
- The Tools page exports the full private build log and latest game log through Android's document picker. It also performs an on-demand version check against this repository's bounded GitHub Releases API response and opens only validated release URLs; it never silently installs an APK.
...
The base game and Retro Rewind now boot and accept touch input on the target tablet. Warm races and heavy menus have been observed at approximately 60 FPS. The complete Android-only base-runtime build has been verified on the target Lenovo tablet: it translated 29,637 functions, compiled 92 ARM64 units, linked an 88,232,312-byte private runtime, and activated it successfully. The Android-only Retro Rewind build is also verified: it translated 29,637 base and 3,837 mod functions, compiled 207 ARM64 units, linked an 88,303,336-byte base library and a 95,896,664-byte Retro Rewind library, activated the combined pack, and removed its successful-build intermediates. Cancellation, thermal, and low-storage behavior still need extended validation. Android background pipeline prewarming uses at most two dedicated low-priority workers, pauses for 45 frames after the render window exceeds 18 ms, restores one worker after recovery, and restores a second only after 120 consistently fast frames. Moderate thermal status pauses prewarming and light status caps it at one worker. Pipelines required by the current frame retain priority access to the remaining worker pool. The Android audio queue is adjustable from 32 to 200 ms and defaults to 64 ms. It remains an experimental port: extended races still need accuracy and ghost comparisons, lifecycle and suspend/resume need stress testing, device-specific audio buffer choices need validation, NAND durability needs repeated interruption tests, controller hotplug and four-player sessions need device testing, and long-session thermals still need measurement. Retro Rewind builds now download and authenticate the signed production Retro-WFC payload, lower it into the generated ARM64 runtime, enable networking for Retro launches, and implement its plaintext service socket path on Android. Install the current Builder and rebuild the combined runtime once; production login, matchmaking, reconnect, and long-session WFC behavior still need tablet validation.

WheelWizard is a .NET 10 Avalonia desktop application. Its mod metadata, priority, launch-overlay behavior, GameBanana catalogue and ZIP downloads, staged Retro Rewind distribution updater, full-SZS conversion, U8/Yaz0 tagged-archive application, Pulsar-compatible BRSAR file-ID composition, controller mapping, video/audio settings, and update discovery now have Android implementations using document URIs and app-private storage. Desktop-only lifetime, executable-launch, URL-registration, window, and setup-EXE behavior is intentionally replaced by Android activities, foreground services, and in-process/JNI installation.
