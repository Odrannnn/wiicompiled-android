...
- A touch GameCube pad supplies analog steering and the main buttons; Mario Kart passes controller detection and reaches the main menu without a physical controller. Bluetooth gamepad motion is routed above the touch overlay and normalized through the same pad bridge so the physical left stick can steer without regenerating the private runtime. An Android **Settings** overlay button toggles the existing in-game settings bar, exposing resolution, graphics, audio, and controller configuration without an F10 keyboard.
